...
public class TicketDAO {
	...
	// 스프링JDBC를 사용하기 위한 멤버변수 와 빈을 주입받기 위한 setter()
	...
	/*
	 	위 2개의 멤버변수는 servlet-context.xml에서 TransactionDAO빈을 생성할 때 초기화 된다.
	 */
	...
	public void buyTicket(final TicketDTO dto) {
		...
		/*
		  	...
		  	구매를 해본다. 6장 이상인 경우 pay테이블에는 insert되지만,
		...
		try {
			...
			// 티켓 구매 처리
			/*
			 	check 제약조건에 의해 countNum이 5를 초과하는 경우에는 제약조건위배로 에러(예외)가 발생된다.
			 */
			template.update(new PreparedStatementCreator() {
				@Override
				public PreparedStatement createPreparedStatement(Connection con) throws SQLException {
			...
			/*
			 	티켓 구매갯수가 5이하인 경우 정상처리되어 모든 작업이 commit 된다.
			 */
			System.out.println("카드결재와 티켓구매 모구 정상처리 되었습니다.");
