...
사용자의 요청을 제일먼저 받는 DispatcherServlet은 기본패키지로 지정된
com.kosmo.k11spring을 스캔하여 컨트롤러 클래스를 찾는다.
그리고 해당 요청명에 매핑되는 메소드를 찾아 요청을 전달하고 비즈니스로직을
수행한다.
...
@RequestMapping
	: 요청명을 매핑한다.
	요청명은 서블릿과 마찬가지로 컨택스트루트를 제외한 나머지경로명으로 이루어진다.
	요청명의 매핑정보로 메소드를 호출하게되므로 메소드명은 큰 의미가 없다.
...
public class FormController {
	...
	/*
	  	폼값받기1] 파라미터로 전달된 값을 HttpServletRequest객체를 통해 받기. 
	  		: Jsp나 Servlet에서 사용한방법과 동일하다.
	 */
	...
	public String loginMember(HttpServletRequest req, Model model) {
		...
		/*
		  	...
		  	ViewResolver가 경로를 조립하여 해당 View를 웹브라우저에 출력하게된다.
	...
	/*
	  	...
	  	조건1 : 쿼리스트링으로 전달되는 파라미터의 갯수와 폼값을
	  		저장할 객체(DTO혹은 VO)의 멤버변수의 갯수가 동일할 때
	  		사용가능함.
	  	...
	  	조건3 : 커맨드객체를 만들 때 파라미터의 갯수는 달라도
	  		상관없으나 파라미터의 이름과 멤버변수의 이름은 반그시
	  		동일해야하고 이에 해당하는 getter/setter가 생성되야 한다.
	  	※ 커맨드객체의 이름을 변경해서 View로 전달하고 싶다면 
	  	@ModelAttribute 어노테이션을 사용하면 된다.(뒤에서 학습)
	 */
	@RequestMapping("/form/commandObjectGet.do")
	...
	/*
	  	폼값받기4] @PathVariable어노테이션으로 폼값받기
	  		...
	  		아래의 경우 2개의 파라미터를 받아서 사용하게된다.
	  		단 웹브라우저는 요청명을 경로로 인식하므로 리소스(이미지 등)를
	  		사용할때는 경로에 주의해야 한다.
	  		파라미터의 갯수가 틀릴경우 404 에러가 발생한다.
	 */
