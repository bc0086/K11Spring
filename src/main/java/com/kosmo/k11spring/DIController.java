...
public class DIController {
	...
	public String myCal(Model model) {		
		...
		/*
		  	...
		  	참조변수에 할당한다. new 연산자를 통해 생성한것과 동일하지만
		  	외부 설정파일에서 미리 생성한 것을 주입(Injection)받은것임에
		  	유의할 것.
		 */
		CalculatorDTO myCal =
		...
		/*
		 	주입받은 빈을 통해 사칙연산을 진행하고, 결과값을 model객체에 저장한다.
		 	해당 결과는 View로 전송한다.
		 */
	...
	public String bmiCal(Model model) {
		...
		/*
		  	사용법 : getBean("빈의 참조변수명", 빈 생성시 사용된 클래스명.class);
		 */
	...
	public ModelAndView myAvengers() {
		...
		/*
		 	오브젝트(객체 혹은 빈)의 생성을 책임지는 오브젝트 팩토리에 대응하는 것이
		 	ApplicationContext이다. 다른말로는 IOC컨테이너, 스프링컨테이너 혹은 BeanFactory로 부르기도 한다.
	...
	// 어노테이션을 활용할 시에는 해당 파일내에서 생성하는 건가??
