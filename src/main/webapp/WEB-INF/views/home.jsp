...
<body>
	...
	<li>
		<a href="./form/commandObjectGet.do?id=kosmo&pw=1234&name=홍길동&email=hong@gildong.com" target="_blank">
	...
	<h2>폼 데이터 검증하기 - Validator</h2>
      <li>
         <a href="./validate/memberRegist.do" target="_blank">Validator</a>
      </li>
...
<!-- 
	...
	4. 컨트롤러에서 외부파일을 읽어서 프로그램에 적용
		...
		4-4) 외부파일을 가져와서 addLast로 추가후 내용읽음
		4-5) 읽은내용을 Model에 저장하거나 비즈니스로직에 직접 사용함.
 -->    
...
<!-- 
	...
	3. 요청명에 따른 컨트롤러를 생성
		- 업로드 폼, 업로드 처리 등을 진행하고 
		- 파일명 변경을 위해 UUID 클래스를 통한 유니크한 문자열 생성
		
	4. 파일이외의 나머지 폼값을 확인하는 페이지 작성
	5. 파일 다운로드를 위해 빈을 생성할 때는 미리 common.FileDownloadView 클래스가
		생성되어 있어야 한다.
 ...
 -->
     ...
     <!-- 
     	트랜잭션의 개념
     		- 인터넷 뱅킹의 경우 A가 B에게 송금을 진행하는 경우 A에서는 출금되었으나 B에게는 입금이 되지 않은
     		상황이 발생된다면 해당 거래는 취소되어야한다.
     		- 이와 같이 양쪽 모두 만족되어야 하나의 프로세스를 완료처리 할 수 있도롣 해주는 기법을 트랜잭션이라 한다.
     		...
     		1. 트랜잭션 매니져를 사용하는 경우
     			...
     			1-3. TicketDAO의 빈을 컨트롤러에서 자동주입받아 사용한다.
     			1-4. 2개 이상의 작업을 try~catch로 묶어 하나라도 오류가 발생하는 경우
     				모든 작업을 rollback시킨다. 모든 작업이 정상처리 되었다면 commit시킨다.
      -->
